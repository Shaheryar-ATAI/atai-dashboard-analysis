...
@push('head')
    ...
    <style>
        ...
        td.pf-cell:empty::before { content: ''; }

        /* Make sure the select actually renders and is visible */
        td.pf-cell .pf-wrap { min-width: 150px; }
...
<div class="position-fixed bottom-0 end-0 p-3" style="z-index: 1080">
    <div id="forecastToast" class="toast align-items-center text-bg-danger border-0" role="alert" aria-live="assertive" aria-atomic="true">
        <div class="d-flex">
...
<main class="container-fluid py-4 forecast-page">
    <div class="glass-row d-flex flex-wrap align-items-center justify-content-between mb-3">
        ...
        <div class="d-flex align-items-center gap-2">
            ...
            <input type="date" class="form-control form-control-sm bg-dark text-light border-0" name="__submission_stub" disabled value="{{ now()->toDateString() }}">
        </div>
    </div>
    ...
    <form id="forecastForm" method="post" action="{{ route('forecast.save') }}">
                        ...
                        <div class="mb-2">
                            ...
                            <div class="d-flex gap-2 flex-wrap">
                                @foreach (['Eastern','Central','Western'] as $r)
                                    <input class="btn-check" type="radio" name="region" id="region-{{ $r }}" value="{{ $r }}" {{ (auth()->user()->region ?? '')===$r ? 'checked':'' }}>
                            ...
                            <div class="col-6">
                                ...
                                <select disabled id="monthSelect" class="form-select form-select-sm">
                                    @foreach([1=>'January',2=>'February',3=>'March',4=>'April',5=>'May',6=>'June',7=>'July',8=>'August',9=>'September',10=>'October',11=>'November',12=>'December'] as $k=>$v)
                                        <option disabled value="{{ $k }}" {{ $k===now()->month ? 'selected':'' }}>{{ $v }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-6">
                ...
                <div class="card section-card h-100">
                    ...
                    <div class="card-body">
                        <div class="mb-2">
                            ...
                            <input name="price_agreed" class="form-control form-control-sm" placeholder="Yes / No / %">
                        </div>
                        <div class="mb-2">
                            <label class="form-label small text-uppercase">Consultant Approval</label>
                            <input name="consultant_approval" class="form-control form-control-sm" placeholder="Approved / Pending">
                        </div>
                        <div class="">
                ...
                <div class="card section-card h-100">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-2">
                            ...
                            <input name="required_turnover" class="form-control form-control-sm" placeholder="SAR 0">
                        </div>
                        <div class="mb-2">
                            <label class="form-label small text-uppercase">Required Forecast</label>
                            <input name="required_forecast" class="form-control form-control-sm" placeholder="SAR 0">
                        </div>
                        <div class="">
                            <label class="form-label small text-uppercase">Conversion Ratio</label>
                            <input name="conversion_ratio" class="form-control form-control-sm" placeholder="e.g. 20%">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-sm align-middle mb-0" id="tblA">
                        ...
                        <tr>
                            <th></th>
                            <th colspan="4" class="text-end text-uppercase small opacity-75">Total New Current Month Forecasted Orders</th>
                            <th class="text-end">
        ...
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-2 mb-5">
            <div class="small text-secondary">
                ...
                Tip: paste from Excel directly into the table (Customer | Products | Project | % | Value | Family | Remarks).
            </div>
            <div class="d-flex gap-2">
                ...
                <button class="btn btn-outline-info d-none js-pdf" type="submit"
                        formmethod="POST" formaction="{{ route('forecast.pdf') }}" formtarget="_blank" formnovalidate  >
      ...
      <td class="text-center"><button type="button" class="btn btn-sm btn-outline-danger del"><i class="bi bi-x-lg"></i></button></td>`;
            ...
            pdfBtns.forEach(btn => {
                btn.addEventListener('click', (e) => {
                    ...
                    setTimeout(() => { if (shouldReloadAfterPdf) window.location.reload(); }, 5000);
                });
            });
            ...
            async function handleSaveClick() {
                ...
                if (res.status === 422) {
                    let data = {};
                    try { data = contentType.includes('application/json') ? await res.json() : {}; } catch(e){}
            ...
            if (formEl) {
                formEl.addEventListener('keydown', (e) => { if (e.key === 'Enter') e.preventDefault(); });
            }
        })(); // end IIFE
        ...
        function markValid(el, ok) { el.classList.toggle('is-invalid', !ok); }
        ...
        function wireQuotationValidation(container){
            ...
            container.querySelectorAll('input.qtn').forEach((el) => {
                if (el.__qtnWired) { el.__qtnSanitize?.(); return; }
                const sanitize = () => {
